...
\newcommand{\Mat}[1]{\mathbf{#1}}
 ...
 \title{sparseHessianFD: An R Package for Estimating Sparse Hessian Matrices}
...
##knitr::opts_chunk[["set"]](prompt="R> ")
##collapse = FALSE, eval=TRUE, echo=TRUE,
...
$f(x)$ symbol).  Let $e_m$ be the $m$th
...
of the $M$ elements of $x$, one at a time.\footnote{More accurate
  ...
  only forward finite differences.}  However, depending
on the sparsity pattern of the Hessian, we might be able to recover
...
even as additional units are added.  How to
decide which variables can be perturbed together is actually a vertex
coloring problem from graph theory (see Section \ref{sec:coloring}).

At the outset, we emphasize that there may be some applications for
...
numerical differentiation, and sparse matrix representation in \proglang{R}.  In Section \ref{sec:using}, we demonstrate how
to use the package, including an example.  Section \ref{sec:timing} includes some time and
accuracy tests.

\section[Background]{Background}\label{sec:background}

Before describing how to use the \pkg{sparseHessianFD} package, we
present two short background notes.  The first is an informal
...
can be reduced by exploiting the sparsity pattern and symmetry.  The
second is a summary of some of the sparse matrix classes that are
defined in the \pkg{Matrix} package.
...
The \citet{PowellToint1979} approach for the general case of sparse Hessians is to partition
...
rows. Column $\Mat{Y}_c$ would have been computed by perturbing two variables at
once, and we would not have been able to solve for any $\hess{im}{}$
...
group 2.  We will postpone the discussion of how to
...
that it is sufficient to solve $\hessLT{}{}\Mat{G}=\Mat{Y}$ instead via a \emph{substitution method}, where $\hessLT{}{}$ is the lower
...
\subsection[Sparse matrices in R]{Sparse matrices in \proglang{R}}\label{sec:Rsparse}
...
\class{sparseMatrix}.  Class names, summarized in Table
\ref{tab:MatrixClasses}, depend on the data type, matrix structure,
storage format.  The levels of these three factors determine the
...
stored.   Values in numeric and logical matrices correspond to the
...
values. \pkg{Matrix} also defines some other classes of
...
\class{dgCMatrix}, even though it is symmetric. Depending on how the
Hessian is used, it might be useful to coerce the Hessian into a
\class{dsCMatrix} object.  Also, the helper functions in Table
\ref{tab:helpers} expect or return certain classes of matrices, so
some degree of coercion of input and output might be necessary.
...
\begin{align}
  ...
  -\frac{1}{2}\left(\beta_i-\mu\right)'\Mat{\Sigma}^{-1}\left(\beta_i-\mu\right)\right)
-\frac{1}{2}\mu'\Mat{\Omega}^{-1}\mu
\end{align}
...
The sparsity pattern of the Hessian consists of the row and column
...
The \pkg{sparseHessianFD} package defines helper functions to convert
between sparse matrices, and the vectors of row and column indices
required by the \class{sparseHessianFD} initializer.  These functions are listed in Table \ref{tab:helpers}.
...
matrix, and extracts the sparsity pattern of its lower triangle.

<<exPattern>>=
library("sparseHessianFD")
bd <- kronecker(diag(3), matrix(T,2,2))
Mat <- as(bd,"sparseMatrix")
printSpMatrix(Mat)
...
intended matrix, use the \pkg{Matrix} \func{sparseMatrix} constructor.
...
(\func{binary.hess}).  The result from \func{binary.hess} is a
  ``true'' value against which we will compare the estimates.  The package also includes sample datasets of different sizes, which
are accessed with the \func{data} function.
...
elements of the lower triangule of the Hessian.  We happen to have a matrix with the same sparsity pattern
of the Hessian we are trying to compute, so we can use the
\func{Matrix.to.Coord} function to extract the appropriate index
vectors.  It is more likely that we would need to determine the row
and column indices manually.
...
In this section, we discuss the computational algorithms for
\pkg{sparseHessianFD}.  The work can be divided into two fundamental
tasks

\begin{enumerate}
\item partitioning the variables into groups;
  \item computing a ``seed matrix'' $\Mat{Y}$ using finite
    differences of gradients, and extracting Hessian values through substitution.
\end{enumerate}
...
columns in $\hessLT{}{}$ have at least one non-zero element in the same row.
...
graph of $\hessLT{\pi}{}$ for some permutation $\pi$.  Furthermore,
...
Now suppose that we were to rearrange $\hess{}{}$ so the last row and
...
estimate the Hessian.  The \class{sparseHessianFD}
class finds a permutation, and partitions the variables, when it is
initialized.  This is all invisible to the end user.  The technical
details of the algorithm are in Section \ref{sec:algorithms}, which should be
...
At this point, we must note that the problem of finding a cycling
coloring of the graph of the sparsity pattern is NP-complete
\citep{ColemanCai1986}.  As such, we will not attempt to find an ``optimal''
partitioning of the variables.  Fortunately, we just need the
partitioning to be reasonably good, to make the effort worth our
while.  A plethora of vertex coloring heuristics have been proposed,
and we make no
claims that any of the algorithms in \pkg{sparseHessianFD} are even ``best available.''  But we do expect them to
generate the correct results faster than the other alternatives.

A partitioning of the variables is consistent with a substitution method if it can be characterized as a cyclic coloring
of the graph of the sparsity pattern, or, equivalently, as a proper
coloring of the intersection graph.  The first step is to permute the rows and columns of the Hessian.   A
reasonable choice is a permutation that sorts the rows and columns in
decreasing order of the number of elements.
To justify this permutation, suppose
...
rows to be smaller, and the intersection graph to be sparser.  This is the ``smallest-last'' ordering suggested by
\citet[Theorem 6.2]{ColemanMore1984}.
...
$\pi$.  The adjacency matrix of the intersection graph is the Boolean
crossproduct, $\hessLT{\pi}{}^\top \hessLT{\pi}{}$.  Algorithm \ref{alg:coloring} is a
``greedy'' vertex coloring algorithm, in which vertices are colored
sequentially.  The result is a cyclic coloring on the sparsity graph, which in turn is a
...
\begin{algorithm}
  \begin{algorithmic}
    ...
    \STATE{$\Mat{B}[C[i], j]\leftarrow B[C[i], j] + z$}
...
estimated Hessians of Equation \ref{eq:LPD} with different numbers of heterogeneous
...
recorded the run times for different tasks in the sparse Hessian
estimation, across 200 replications.  The tasks are summarized in Table \ref{tab:timeMeasures}.
