...
across units are not.  Exactly which elements are  Thus, we can determine exactly which elements
...
sparse Hessians using finite differences.  The specifics of the method
are discussed in Section XX, but the basic idea is as follows.  Let
$f(x)$ be a function to be differentiated, let $D(x)$ be the gradient,
and let $H(x)$ be the Hessian.  Also, let $e_k$ be the $k$th
coordinate vector, and let $\delta$ be a sufficiently small scalar
constant.  One can approximate the $k$th column of an otherwise
unstructured dense Hessian as $\left(G(x+\delta
  e_k)-G(x)\right)/\delta$.  Estimating the $K$ columns of the fill
Hessian involves $K+1$ evaluations of the gradient.  If, however, we
know that some elements of the Hessian are zero, we can perturb more
than one elements of $x$ at once, and extract the non-zero values by
solving a system of linear equations.  Not all sparsity patterns lend
themselves to reducing the number of gradient evaluations, but for
hierarchical models with conditionally independent units, the number
of gradient evaluations is \emph{constant}, no matter how many heterogeneous
units are in the dataset.
...
Instead, place a multivariate normal prior on $\mu$, with mean $0$ and covariance $\Omega_0$.  Thus, each $\beta_i$, and $\mu$ are $k-$dimensional vectors, and the total number of unknown variables in the model is $(N+1)k$.
...
\item  columns  $i_m$ and another other $i_{m'}$ are in the same
  ...
  ordering induces a substitution
  method if and only if either
  \begin{itemize}
  \item if two columns with the same color have an element in the same
    row,
  \end{itemize}
