...
\subsubsection{Conversion of sparse matrices}\label{sec:conversion}
...
\caption{Computation times (milliseconds) for computing Hessians using the
  numDeriv and sparseHessianFD packages.  Rows are ordered by the
...
\bibliography{/Users/mbraun92/OneDriveBusiness/References/Papers/braun_refs}
