...
\item The gradient can
  ...
  when computing finite differenced Hessians, for two reasons.  The
...
more accurate than estimating the Hessian matrix numerically.
Even when we have derived and coded the Hessian matrix symbolically, in practice it may still be faster to estimate the Hessian
...
can be reduced by exploiting the sparsity pattern and symmetric structure.  The
...
\Citet{PowellToint1979} describe a method for estimating the
general case of sparse Hessians that partitions
...
rows. The corresponding column $\Mat{Y}_c$ is computed by perturbing two variables at
...
The algorithms in \pkg{sparseHessianFD} assign variables to groups
...
zero. Class names, summarized in Table
...
The \pkg{sparseHessianFD} package defines helper functions (Table \ref{tab:helpers}) to convert
...
(\func{binary.hess}) of this model. We will treat the result from \func{binary.hess} as a
  ...
  estimates.  The package also includes sample datasets of different
  sizes, which are accessed with the \func{data} function.
...
elements of the lower triangule of the Hessian.  We happen to have a
...
associated graph with a proper vertex coloring.  Every column (and thus, every pair of columns) in Figure \ref{fig:graph1adj} has a non-zero element
...
elements \citet[Theorem 6.2]{ColemanMore1984}. To justify this permutation, suppose
