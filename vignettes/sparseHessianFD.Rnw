...
We implement a ``smallest-last'' ordering.

\begin{algorithm}
  \caption{Algorithm to generate a substitution-consistent coloring of variables}
  \label{alg:color1}
  \begin{algorithmic}
    \STATE{$P$ is a list of sets, where $P[i]$ is the set of
      non-zero column indices in row $i$}
    \STATE{Initialize $D$ with degrees on graph}
  \STATE{$k\leftarrow 0$}
  \STATE{$S\leftarrow\{1,/mathellipsis,K\}$}\COMMENT{indices of all variables}
  \WHILE{$S$ is not empty}
      \STATE{Copy $S$ to $A$}
      \WHILE{$A$ is not empty}
          \STATE{$r\leftarrow$ variable id with largest degree in
            current graph}
          \STATE{Insert $r$ in $W[k]$}
          \STATE{Remove $r$ from $S$}
          \FOR{$a$ in all members of $A$}
              \STATE{$N\leftarrow P[r]\cap a$}
              \IF{$N$ is not empty}
                  \STATE{$D_j\leftarrow 0$}
                  \STATE{Remove $A_j$ from $A$}
                \ENDIF
                \STATE{Move $j$ to next element of $A$}
          \ENDFOR
          \STATE{Remove all variables from $P[r]$}
          \ENDWHILE
          \STATE{$D\leftarrow 0$}
          \FOR{$i$ in all members of $S$}
              \FOR{$j$ in all members of $W[k]$}
                  \STATE{Remove $j$ from $P[i]$}
                  \STATE{$D\leftarrow$ degree of $P[i]$}
              \ENDFOR
          \ENDFOR
