...
of the $M$ elements of $x$, one at a time.\footnote{More accurate
  ...
  only forward finite differences in this paper.}  However, depending
...
\begin{align}
  \label{eq:LPD}
  \log \pi(\beta_{1:N},\mu|\Mat{Y}, \Mat{X}, \Mat{\Sigma},\Mat{\Omega})=\sum_{i=1}^Np_i^{y_i}(1-p_i)^{T-y_i}
  -\frac{1}{2}\left(\beta_i-\mu\right)'\Mat{\Sigma}^{-1}\left(\beta_i-\mu\right)
-\frac{1}{2}\mu'\Mat{\Omega}_0^{-1}\mu
\end{align}
...
Hessian.  Since the $\beta_i$ are independently distributed, and the $y_i$ are
...
\subsection[Sparse matrices in R]{Sparse matrices in \proglang{R}}

The \pkg{sparseHessianFD} package uses the sparse matrix classes that are defined
in the \pkg{Matrix} package \citep{R_Matrix}. We refer the reader to that class's documentation for
all of the details, but we want to highlight a few important points.


\subsubsection[Class names and inheritance]{Class names and inheritance}
...
\begin{table}[ptb]
    \begin{tabularx}{1.0\linewidth}{ll|lll}
      \toprule
...
\class{dsCMatrix} object.  Also, the helper functions in Section
\ref{sec:conversion} expect or return certain classes of matrices, so
...
printSpMatrix(A)
A@x
sparseHessianFD::Matrix.to.Pointers(A, order="column", index1=TRUE)
@
...
\subsubsection[Conversion of sparse matrices]{Conversion of sparse matrices}\label{sec:conversion}
...
\subsection[Numerical differentiation]{Numerical differentiation}\label{sec:numdiff}
...
$\hess{}{}$.  The values in $\Mat{Y}_c$ are finite differences after
...
$\Mat{Y}$, and the second is to extract the values of $\hess{}{}$
...
triangle of $\pi\hess{}{}{\pi^\top}$.    \Citet[Theorem 6.1]{ColemanMore1984} show that a coloring is
triangular if and only if it is also a coloring of the intersection
graph of $\hessLT{\pi}{}$ for some permutation $\pi$.  Furthermore, \citet{ColemanCai1986} prove that a partitioning is consistent with a
...
\subsection[Providing the sparsity pattern]{Providing the sparsity pattern}
...
the sparsity pattern is correct.  In practice, rather than trying to keep track of the row and column indices
...
row and column indices from a sparse matrix.
...
Mat <- as(kronecker(diag(3), matrix(T,2,2)),"nMatrix")
printSpMatrix(Mat)
...
\subsection[Evaluating the Hessian]{Evaluating the Hessian}
...
not need to be supplied again. Similarly, the \func{hessian} method takes a single argument.
The return value is always a \class{dgCMatrix} object.  The
...
of the Hessian we are trying to compute, we can use the
...
and column indices manually, as we did in Section \ref{sec:sparsity}.
...
of the data set increases.  We can conclude that the
...
\subsection[Partitioning the variables]{Partitioning variables}
