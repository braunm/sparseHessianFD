...
\end{enumerate}
  ...
  \begin{align}
    \label{eq:1}
    f(x+h)&=f(x) + f'(x)\delta + \frac{1}{2}f''(x)\delta^2 + o(\delta^3)\\
  \end{align}
...
first proposed in \citet{CurtisPowell1974} for Jacobian matrices, and described in \citet{PowellToint1979}. To begin, partition the variables
...
Note that this system is overdetermined; \citet{CurtisPowell1974} did
...
Both \citet{CurtisPowell1974} (for direct methods) and
