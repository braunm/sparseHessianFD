...
The \pkg{sparseHessianFD} package provides an interface to ACM TOMS Algorithm
...
\begin{enumerate}
\item an R function that returns the value of the objective function
whene valuated at a variable vector $P$;
\item an R function that returns the gradient of that objective function
at $P$; and
\item the row and column indices for the non-zero elements in the lower triangle of the Hessian.
\end{enumerate}
