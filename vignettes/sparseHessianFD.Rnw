...
Also, we have found in practice that even when we have derived and
coded the
Hessian matrix symbolically, sometimes it can still be faster to estimate the Hessian
using \pkg{sparseHessianFD} than running coding it directly (maybe
because of our weak coding skills, or the computation time to compute
intermediate values).   These are the
...
(\variable{FALSE}). If `order.row=TRUE`, then the Hessian
...
@
    ...
    \section{Underlying theory and algorithms}\label{sec:theory}
  ...
  $M$-dimensional vectors.  Let $u$ be a sufficiently small positive
...
\end{align}
with $\parD{}{}{x}=\left(\parD{}{1}{x},\mathellipsis,\parD{}{M}{x}\right)$ as
...
into $C$ mutually exclusive groups, or ``colors.'' so $c_k$ indexes
the color of variable $m$. Let $\Mat{G}$ and $\Mat{Y}$ be $M\times C$
matrices, where $\Mat{G}_{kc}=\delta$ if variable $m$ belongs to group $c$, and zero
otherwise, and let $G_c$ be the $c$th column of $G$.  Each column in $Y$ is defined as
\begin{align}
  \label{eq:Yg}
  Y_c&=\parD{}{}{x+G_c}-\parD{}{}{x}
\end{align}
...
choose $C$ and how to color the variables.
...
\citep{ColemanCai1986,GebremedhinTarafdar2007,GebremedhinManne2005},
...
\begin{algorithm}
  ...
  \label{alg:color1}
  \begin{algorithmic}
    \REQUIRE{Rows and columns are labeled as consecutive integers $1,\mathellipsis,M$}
    \REQUIRE{$P,Q$, are lists of $M$ sets (initially identical), where
...
\FOR{$i$ = 1 \TO $M$}
  ...
  \STATE{$c\leftarrow 0$}
  \STATE{Insert $\{1,\mathellipsis,M\}\rightarrow S$}\COMMENT{indices of all variables}
    \STATE{Initialize set $W[c]$}
  \WHILE{$S$ is not empty}
      \STATE{Copy $S\rightarrow A$}
      \WHILE{$A$ is not empty}
          \STATE{$r\leftarrow$ index of $\max(D)$.}
          \STATE{Insert $r\rightarrow W[c]$}
