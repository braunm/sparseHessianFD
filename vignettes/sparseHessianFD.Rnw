...
Hessian.  Since the $\beta_i$ are MVN-distributed, and the $y_i$ are
...
have one-based indexing, and stores $X$ in CSC format.  We can then
...
the first column, in row \code{iRow[2]=4}, so $A_{41}=7$.  Elements 3 and 4 in
...
5. Hence, $A_{22}=x_3=8$ and $A_{32}=x_4=9$.
Column 3 begins with $A_{33}=x_5=10$ (\code{iRow[5]=3} gives us the
row index).  Continuing,
$A_{44}=x_6=11$.  The final value in
\variable{jpntr} is always the number of non-zeros in the matrix.  The differences between
adjacent elements in \variable{jpntr} are the number of non-zeros in each column of $A$.
...
diagonal element.  The matrix equation $\hess{}{}G=Y$ represents the linear approximation
...
have the following system of linear equations from $\hess{}{f(x)}\Mat{G}=\Mat{Y}$.
\begin{align}
  \label{eq:11}
  \begin{aligned}[c]
  h_{11}&=y_{11}\\
...
that it is sufficient to solve $\hessLT{}{}G=Y$ instead via a \emph{substitution method}, where $\hessLT{}{}$ is the lower
...
contrast with the six that would have been needed had $\hess{}{f(x)}$ been treated
as dense.

The $\Mat{Y}$ matrix is, in essence, a compressed representation of
$\hess{}{}$.  The values in $Y_c$ are finite differences after
...
Hessian/Gradient&ratio of the Hessian time to the
    gradient time
  \end{tabularx}
  \caption{Summary of timing tests (see Figure )}\label{tab:timeMeasures}
\end{table}
...
\begin{align}
  \label{eq:2}
  \hess{ij}{f(x)}&=Y_{i,C_j}/\delta - \sum_{l>i,l\in C_j}\hess{li}{f(x)}
\end{align}
