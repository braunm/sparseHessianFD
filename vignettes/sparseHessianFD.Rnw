...
derivative), and its Hessian $\hess{}{f(x)}$.  Let $e_m$ be the $m$th
coordinate vector, and let $\delta$ be a sufficiently small scalar
constant. The vector $\hess{m}{f(x)}\approx\left(\parD{}{}{f(x+\delta e_m)}- \parD{}{}{f(x)}\right)/\delta$ is
a linear approximation to $\hess{m}{f(x)}$, the $m$th column of the
...
Mat <- as(Matrix::kronecker(diag(N),matrix(1,k,k)),"nMatrix")
...
\begin{subfigure}[b]{.5\textwidth}
<<banded, echo=FALSE>>=
Mat <- as(Matrix::kronecker(Matrix(1,k,k), diag(N)),"nMatrix")
Mat <- rBind(Mat, Matrix(TRUE,k,N*k))
Mat <- cBind(Mat, Matrix(TRUE, k*(N+1), k))
print(Mat)
@
...
To estimate the $k$th column of $\hess{}{f(x)}$, we again choose a
sufficiently small $\delta$, and compute
\begin{align}
  \label{eq:1}
  \hess{k}{f(x)}&\approx\frac{\parD{}{}{f(x+\delta e_k)}-\parD{}{}{f(x)}}{\delta}
\end{align}
...
diagonal element.  The matrix equation $\hess{}{f(x)}G=Y$ represents the linear approximation
$\hess{im}{f(x)}\delta\approx y_{im}$, and we can solve for all elements of $\hess{}{f(x)}$
...
once, and we would not have been able to solve for any $\hess{im}{f(x)}$
...
that it is sufficient to solve $\hessLT{}{f(x)}G=Y$ instead via a \emph{substitution method}, where $\hessLT{}{f(x)}$ is the lower
triangular part of $\hess{}{f(x)}$.  This has the effect of removing the equations
...
$\hess{}{f(x)}$.  The values in $Y_c$ are finite differences after
...
$\Mat{Y}$, and the second is to extract the values of $\hess{}{f(x)}$
...
the non-zero elements in each column of $\hessLT{}{f(x)}$.  In the
intersection graph, two vertices are connected if the corresponding
columns in $\hessLT{}{f(x)}$ have at least one non-zero element in the same row.
...
permutation of the rows and columns of $\hess{}{f(x)}$.  Let $\pi$
represent such a permutation, and let $\hessLT{\pi}{f(x)}$ be the lower
triangle of $\pi\hess{}{f(x)}{\pi^\top}$.    \Citet[Theorem 6.1]{ColemanMore1984} show that a coloring is
triangular if and only if it is also a coloring of the intersection
graph of $\hessLT{\pi}{f(x)}$ for some permutation $\pi$.  Furthermore, \citet{ColemanCai1986} prove that a partitioning is consistent with a
...
for the intersection graph of $\hessLT{\pi}{f(x)}$.
...
Now suppose that we were to rearrange $\hess{}{f(x)}$ so the last row and
