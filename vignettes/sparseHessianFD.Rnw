...
graph theory.  The ``intersection set'' of two columns of
$\hessLT{}{x}$ consists of the row indices of non-zero elements that
are common to both columns.  In the ``intersection graph,'' two
variables are connected if their intersection set is non-empty.  Thus,
two columns are connected in the intersection graph if they have any
non-zero element in a common row.

The intersection graph of $\hessLT{}{x}$ is \emph{not} invariant to
permutation of the rows and columns of $\hess{}{x}$.  Let $\pi$
represent such a partition, and let $\hessLT{\pi}{x}$ be the lower
triangle of $\pi\hess{}{x}{\pi^\top}$.

A ``proper'' coloring of a graph is one in which two
vertices with a common edge do not have the same color. \Citet{ColemanMore1984} define a ``triangular coloring'' of a graph is
...
uses at least three colors \citep{GebremedhinTarafdar2007}. \Citet{ColemanCai1986} prove that a partitioning is consistent with a
...
\path node at (0.5, 0) [fill=blue!20] (v1) {1};
\path node at  (1.5, 0) [fill=red!20] (v2) {2};
\path node at  (2.5, 0) [fill=blue!20] (v3) {3};
\path node at  (3.5, 0) [fill=red!20] (v4) {4};
\path node at  (4.5, 0) [fill=blue!20] (v5) {5};
\path node at  (5.5, 0) [fill=red!20] (v6) {6};
\path node at  (3, 1.5)  [fill=green!20] (v7) {7};
\draw
(v1) -- (v7)
(v2) -- (v7)
(v3) -- (v7)
...
\begin{tikzpicture}
\tikzstyle{every node}=[draw, shape=circle]
\path node at (0, 3) [fill=blue!20] (v1) {1};
\path node at  (0, .75) [fill=red!20] (v2) {2};
\path node at  (2, 3) [fill=green!20] (v3) {3};
\path node at  (2, .75) [fill=purple!20] (v4) {4};
\path node at  (4, 3) [fill=cyan!20] (v5) {5};
\path node at  (4, 0) [fill=white!20] (v6) {6};
\path node at  (3, 1.5)  [fill=yellow!20] (v7) {7};
\draw
(v1) -- (v7)
(v2) -- (v7)
(v3) -- (v7)
...
The \pkg{sparseHessianFD} package groups variables by permuting the
rows and columns of the Hessian and coloring the intersection graph of
the lower triangle of the permuted matrix.
...
Once the colors are established, they no longer need to be recomputed.
