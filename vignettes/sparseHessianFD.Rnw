...
The time measures are:

\begin{enumerate}
\item[Function] estimating the objective function;
\item[Gradient] estimating the gradient;
\item[Hessian] computing the Hessian (not including initialization or
  partitioning time);
   \item[Partitioning] finding a valid partitioning of the
  variables (the vertex coloring problem);
\item[Initialization] total setup time (including the partitioning time);
  \item[Hessian/Gradient] ratio of the Hessian time to the
    gradient time
\end{enumerate}
