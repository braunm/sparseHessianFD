...
constant. The vector $\left(Df(x+\delta e_k) - Df(x)\right)/\delta$ is
...
derivative.  If the Hessian is sufficiently sparse, we can perturb
more than one element of $x$ at a time and still recover the non-zero
Hessian values.   Not all sparsity patterns allow for fewer than $K$
...
At the
outset, we have to mention that there may be some applications for
...
\begin{enumerate}
\item The objective function should be twice differentiable, and can be
...
\item The first derivative vector (or its transpose, the gradient) can
  ...
  compute the gradient.  Otherwise, the Hessian is approximated using
  finite differences of finite differences, and the approximation
  error is compounded.
...
\item Preferred alternatives to computing the Hessian are not
  ...
  tedious or difficult to implement. Algorithmic differentiation
  should also be considered first.  \pkg{sparseHessianFD} makes the
  most sense when the gradient is easy to get, but the Hessian is
  not.
