...
more accurate than estimating the Hessian numerically.
Even when we have derived and coded the Hessian symbolically, in practice it may still be faster to estimate the Hessian
...
In this section, we discuss the algorithms that
\pkg{sparseHessianFD} uses for partitioning the variables, and
estimating the non-zero values in the Hessian.
