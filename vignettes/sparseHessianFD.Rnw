...
<<>>=
Mat <- as(kronecker(diag(N),matrix(1,k,k)),"lMatrix")
...
<<>>=
Mat <- as(kronecker(matrix(1,k,k), diag(N)),"lMatrix")
Mat <- rBind(Mat, Matrix(TRUE,k,N*k))
Mat <- cBind(Mat, Matrix(TRUE, k*(N+1), k))
print(Mat)
@
...
Now let's consider a general case, starting with a described in \citet{PowellToint1979}. To begin, partition the variables
into $C$ mutually exclusive groups, or ``colors.'' so $c_m$ indexes
...
pattern of the Hessian. Consider and example with the following values and
sparsity pattern.
...
choose $C$ and how to color the variables, until Section \ref{sec:coloring}.
...
that it is sufficient to solve $\hessLT{}{x}G=Y$ via a substitution method, where $L$ is the lower
...
\subsection{Coloring the variables}\label{sec:coloring}
...
their respective columns have a non-zero element in the same row in
the lower triangle.  Certainly the grouping in the last example meets that criterion, but
it is not the only possible grouping.  A valid, yet trivial
...
lower triangle of the Hessian represents an edge between two
vertices. \Citet{ColemanCai1986} proved that a coloring is consistent
if and only if the coloring is \emph{cyclic}:  any cycle in the graph
must use at least three colors.
...
see that the coloring is cyclic.
...
A reasonable question to ask next would be if there is an algorithm
that can color the variables with a minimal number of colors.  It
turns out that the cyclic coloring problem is NP-hard
\citep{ColemanMore1984,ColemanCai1986}, so we have to rely on
heuristics.  There are quite a few options from which to choose
\citep{PowellToint1979,ColemanCai1986,GebremedhinTarafdar2007}, all of
which vary into their ability to minimize the number of colors, but
also in their run time and ease of implementation.  And since there is
no solution to the \emph{optimal} coloring problem, we are happy with
coloring algorithms that are ``pretty good.''

For \pkg{sparseHessianFD}, that means the original
\citet{PowellToint1979} method. (Explain why here).



\subsubsection{Algorithm and Permutations}
