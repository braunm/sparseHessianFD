...
make_funcs <- function(D, priors, order.row) {
...
run_test <- function(NkTord, reps=50) {

    N <- as.numeric(NkTord["N"])
    k <- as.numeric(NkTord["k"])
    T <- as.numeric(NkTord["T"])
    ord <- as.logical(NkTord["ord"])

    data <- binary_sim(N, k, T)
    priors <- priors_sim(k)
    F <- make_funcs(D=data, priors=priors, order.row=ord)
    nvars <- N*k+k
    ...
    cat("Completed N = ",N,"\tk = " , k , "\tT = ",T,"\torder = ",ord,"\n")
