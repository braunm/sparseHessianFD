...
\begin{Schunk}
\begin{Sinput}
R> f <- obj$fn(x)          ## returns numeric
R> df <- obj$gr(x)         ## returns numeric vector
R> hess <- obj$hessian(x)  ## returns dgCMatrix
R> fngr <- obj$fngr(x)     ## returns list
R> fngrhs <- obj$fngrhs(x) ## returns list
\end{Sinput}
\end{Schunk}
