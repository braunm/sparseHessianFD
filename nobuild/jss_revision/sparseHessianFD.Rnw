...
When $f(x)$ is complex analytic, we can approximate $\parD{}{}{f(x)}$
and $\hess{}{f(x)}$ using the complex step
method. To do this, we extend our defintion of a real $x\in\Real{M}$
to be a complex $x\in\Complex{M}$ for which the imaginary part is
zero. We then take a Taylor
series expansion of $f(x)$ in the imaginary direction of the complex
plane \citep{SquireTrapp1998}, rearrange terms, and take the imaginary
parts of both sides.
\begin{align}
...
require a differencing operation, but a second-order approximation of
the Hessian does \citep{AbreuStich2013}.  However, when the gradient
is provided, we can estimate the Hessian as if it were a Jacobian of
the vector-valued gradient function \citep{LaiCrassidis2008}.
...
pattern as the Hessian in Equation~\ref{eq:FDhess2sp}? First, we need
to make an assumption that structural zeros remain zero for all
...
Hessian. But for all complex $x$, the sparsity constraints ensure that
...
for $x_1$, it must also hold for $x_1+i\delta$, because Equation~\ref{eq:I1d} holds
for $x_2$, it must also hold for $x_2+i\delta$. Thus, for real $x$,
...
\begin{align}
  ...
  \Mat{Y}_c&=\Imag(\nabla f(x+iG_c))
\end{align}
The advantage of this method is that there is no longer a
