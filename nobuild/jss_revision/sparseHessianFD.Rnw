...
Suppose we have a dataset of $N$ households, each with $T$
 ...
 opportunities, so $y_i$ is a binomial random variable.  Define each
 $p_i$ such that it depends on a covariate vector
 $x_i\in\Real{k}$, and a heterogeneous coefficient vector $\beta_i\in\Real{k}$.
\begin{align}
...
covariance $\Mat{\Sigma}\in\Real{k\times k}$.   Assume that we know $\Mat{\Sigma}$, but not $\mu$.
Instead, place a multivariate normal prior on $\mu$, with mean $0$ and
covariance $\Mat{\Omega}\in\Real{k\times k}$.  Thus, each $\beta_i$, and $\mu$ are
