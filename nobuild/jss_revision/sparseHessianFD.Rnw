...
of the $M$ elements of $x$, one at a time.\footnote{More accurate
  approximations using finite differences require more gradient
  evaluations.  In this section, we will consider
...
A call to the \class{sparseHessianFD} \method{hessian} method will
apply the complex step method if the 
\funcarg{complex=TRUE} was set in the initializer. The functions
...
results were generated on an Apple Mac Pro with a 2.7 GHz
12-core Intel Xeon E5 processor and 64 GB of RAM, running MacOS
version 10.12.2, and \proglang{R} version 3.3.2.
