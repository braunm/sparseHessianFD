...
$\parD{2}{\beta_i, \mu}=0$ (because $\mu$ is the prior mean of each
...
Evaluations of the function and gradient will generate the same
results as the true values because they are computed using the same function.
...
Note that variables \variable{f} and \variable{true.f} were computed
using the same exact calculation, as were variables
\variable{gr} and \variable{true.grad}.In contrast,
\variable{true.hess} was computed using an exact calculation, while
\variable{hs} is the sparse finite difference approximation.

were computed by the same functions
...
\subsection[Complex step method]{Complex step method}
...
In short, the complex step method is available to users who want
improved accuracy and speed, and know what they are doing.
