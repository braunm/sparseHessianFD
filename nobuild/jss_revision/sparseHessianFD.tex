...
of the $M$ elements of $x$, one at a time.\footnote{More accurate
  approximations using finite differences require more gradient evaluations.  We will consider
  only forward finite differences. In Section \ref{sec:complex} we
  introduce the complex step method that, under certain conditions,
  can generate more accurate estimates with the same number of
  gradient evaluations using forward finite differences.} In a sparse matrix, most of the
...
function, its gradient, and the sparsity pattern of its Hessian
matrix. The gradient should be computed exactly (within machine
precision), as opposed to being numerically estimated (e.g., by finite
differencing). The sparsity pattern (e.g., location of structural
...
tool for the job. In general, finite differencing is not a ``first
choice'' method because the result is not exact, so
...
the Hessian will be estimated numerically. However, deriving a vector of first
...
Next, define $\Mat{Y}\in\Real{M\times C}$ such that each column is a
...
The evaluation of the Hessian is a finite differenced approximation,
...
\subsection[Complex step method]{Complex step method}\label{sec:complex}

The \pkg{sparseHessianFD} package also allows for
estimation of the Hessian using the complex step method
\citep{SquireTrapp1998} as an alternative to finite differencing. In
this method, the definition of $\Mat{Y}_c$ in Equation \ref{eq:Yg} is replaced with
\begin{align}
  \label{eq:complexYg}
  \Mat{Y}_c&=\Imag(\nabla f(x+iG_c))
\end{align}
where $i=\sqrt{-1}$ and $\Imag$ returns the imaginary part of a complex
number. The advantage of this method is that there is no longer a
subtraction operation to generate truncation error. Thus, $\delta$ can
be made arbitrarily small, and the resulting estimate of the Hessian
is more accurate.

To use the complex step method, include the argument
\funcarg{complex=TRUE} in the call to the initializer of the \class{sparseHessianFD}
object. The functions passed as \funcarg{fn} and \funcarg{gr} must both
...
for basic arithmetic, trigonometric, logarithmic and exponential
functions, many other common functions (e.g.,  \func{gamma}, \func{log1p},
...
only if the function is holomorphic
(complex differentiable). The methods in \class{sparseHessianFD} do
...
The log posterior density in Equation \ref{eq:LPD} is holomorphic, so we
...
\pkg{sparseHessianFD}.  All of the subsequent speed and scalability
results were generated on a 2013-vintage Apple Mac Pro with a 2.7 GHz
12-core Intel Xeon E5 processor and 64 GB of RAM.
...
and standard deviations (across 20 replications) for the time (in
milliseconds) to compute a Hessian using each package.  The difference in run times is dramatic,
especially because the computation time for \pkg{numDeriv} grows
quadratically in the number of variables. Times
were collected using the \pkg{microbenchmark} package
\citep{R_microbenchmark}. Code to replicate Table~\ref{tab:numDeriv}
is in the file \filename{replication.R}, which is available as an
